...
public class InterventionCutscene : MonoBehaviour
{
    ...
    private IEnumerator PlayCutscene()
    {
        ...
        LookLeft(adultAnimator);
        LookRight(fatherAnimator);
        yield return new WaitForSeconds(1f);
        yield return StartCoroutine(StartDialogue(0));
        yield return new WaitForSeconds(1f);
        StartMoving(fatherAnimator);
        LookLeft(fatherAnimator);
        yield return MoveTransformToTarget(fatherTransform, new Vector3(-8.92f, -3.82f));
        LookUp(fatherAnimator);
        yield return MoveTransformToTarget(fatherTransform, new Vector3(fatherTransform.position.x, 1.47f));
        StopMoving(fatherAnimator);
        father.SetActive(false);
        StartMoving(motherAnimator);
        LookLeft(motherAnimator);
        yield return MoveTransformToTarget(motherTransform, new Vector3(-9f, motherTransform.position.y));
        LookUp(motherAnimator);
        yield return MoveTransformToTarget(motherTransform, new Vector3(motherTransform.position.x, 1.47f));
        StopMoving(motherAnimator);
        mother.SetActive(false);
        yield return new WaitForSeconds(2f);
        childAnimator.SetBool("isFallen", false);
